...
# run like:
#
#python calculate_vorticity_mask.py ../sims/modern/output/hdf5/ shapefile.shp test.csv -v
#
# Does not work in parallel currently
...
def main():
    ...
    parser.add_argument(
            "shapefile",
            help="A polygon shapefile with the area you want vorticity calculated within"
            )
    ...
    spfile = args.shapefile

    rank = MPI.COMM_WORLD.Get_rank()

    mask_area = gp.read_file(spfile)

    def in_mask(point):
        point = Point(point[0], point[1])
        if mask_area.contains(point).any():
            return 1
        else:
            return 0
    ...
    # we can work out our mask function first
    # note there are no checks here! 
    with CheckpointFile(h5_files[0], "r") as f:
        mesh2d = f.load_mesh()
        function = f.load_function(mesh2d,"uv_2d")
        # create a p1-cg space for vorticity
        mask = Function(FunctionSpace(mesh2d, 'CG', 1), name="mask")
        xvector = mesh2d.coordinates.dat.data
        mvector = mask.dat.data
        for i, (xy) in enumerate(mesh2d.coordinates.dat.data):
            mvector[i] = in_mask(xy)


    # mask is now 1 in the polygon and 0 outside
